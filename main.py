...
c_minor_signal, sr = librosa.load(c_minor_audio_path)
c_minor_to_other_signal, sr = librosa.load(c_minor_andother_audio_path)
future_ski_signal, sr = librosa.load(future_ski)

plt.figure(figsize=(18, 8))
librosa.display.waveshow(signal, sr=sr, alpha=0.5)

# Derive spectrum using Fourier Transform
ft = sp.fft.fft(c_minor_signal)  
...
# Output the prominent frequencies
print("Prominent Frequencies (Hz):", prominent_frequencies)
...
plt.title("Fourier Transform C Minor")
plt.show()
...
print("Frequencies for C Minor:", filtered_prominent_notes)


# Derive spectrum using Fourier Transform
ft = sp.fft.fft(c_minor_to_other_signal)
magnitude = np.absolute(ft)
#frequency = np.linspace(0, sr, len(magnitude))
frequency = np.fft.fftfreq(len(magnitude), 1/sr)
...
plt.xlabel("Frequency (Hz)")
plt.ylabel("Magnitude")
plt.title("Fourier Transform C Minor to other")
plt.show()
...
#print("Corresponding Notes:", notes)
print("Frequencies for C Minor to other:", filtered_prominent_notes)
